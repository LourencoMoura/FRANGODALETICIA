...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (tabFromUrl && ['orders', 'promotions', 'customers', 'products', 'settings'].includes(tabFromUrl)) {
  ...
  // tRPC Queries
  const { data: ordersList, isLoading: isFetchingOrders } = trpc.orders.list.useQuery();
  const { data: customersList, isLoading: isFetchingCustomers } = trpc.customers.list.useQuery();
  ...
  const stats = {
    total: orders.length,
    pending: orders.filter(o => o.status === "pedido-recebido" || o.status === "preparando").length,
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-6 border-l-4 border-l-orange-500 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Total de Pedidos</p>
          ...
          <Card className="p-6 border-l-4 border-l-yellow-500 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Pendentes</p>
          ...
          <Card className="p-6 border-l-4 border-l-green-500 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Prontos</p>
                <p className="text-3xl font-black text-gray-800">{stats.ready}</p>
              </div>
          ...
          <Card className="p-6 border-l-4 border-l-blue-500 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Entregues</p>
                <p className="text-3xl font-black text-gray-800">{stats.delivered}</p>
              </div>
        ...
        <div className="mt-8">
            ...
            <Card className="p-6 shadow-md border-orange-100 italic-none">
              ...
              {loading ? (
                ...
                <div className="text-center py-20 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                  <p className="text-gray-400 font-medium">Nenhum pedido encontrado no momento.</p>
                </div>
              ) : (
                <div className="overflow-x-auto rounded-lg border">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">ID</th>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">Total</th>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">Pagamento</th>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">Status Atendimento</th>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">Ações</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      {filteredOrders.map((order) => (
                        <tr key={order.id} className="hover:bg-orange-50/30 transition-colors">
                          <td className="py-4 px-4 font-mono font-bold text-orange-600">#{order.id}</td>
                          <td className="py-4 px-4 font-black">R$ {order.total.toFixed(2)}</td>
                          <td className="py-4 px-4">
                            <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase ${
                              ...
                              {order.paymentStatus === 'approved' ? 'Aprovado' : 
                               order.paymentStatus === 'pending' ? 'Pendente' : 'NP'}
                            </span>
                          </td>
                          <td className="py-4 px-4">
                            <select
                            ...
                            >
                              ...
                              <option value="pronto">Pronto / Aguardando</option>
                              <option value="saiu-para-entrega">Saiu para Entrega</option>
                              <option value="entregue">Finalizado</option>
                            </select>
                          </td>
                          <td className="py-4 px-4 text-right">
                            <Button size="sm" variant="ghost" className="text-orange-600 hover:text-orange-700 font-bold">
                              Detalhes
                            </Button>
                          </td>
                        </tr>
               ...
               {isFetchingCustomers ? (
                ...
                <div className="overflow-x-auto border rounded-xl">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">Nome / Apelido</th>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">WhatsApp</th>
                        <th className="text-left py-4 px-4 font-bold text-gray-600 text-sm">Saldo Fidelidade</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      {customersList.map((customer: any) => (
                        <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                          <td className="py-4 px-4">
                            ...
                            <p className="text-xs text-gray-400">@{customer.apelido}</p>
                          </td>
                          <td className="py-4 px-4 text-gray-600 font-medium">{customer.whatsapp}</td>
                          <td className="py-4 px-4">
                            <div className="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 rounded-full border border-amber-100">
                              <Gift className="w-3 h-3 text-amber-500" />
                              <span className="font-black text-amber-700 text-sm">{customer.points} pts</span>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {activeTab === 'products' && (
            <div className="bg-white rounded-2xl p-16 text-center shadow-xl border-2 border-dashed border-orange-100 flex flex-col items-center">
              ...
              <h3 className="text-2xl font-black text-gray-800 mb-3">Gestão de Cardápio</h3>
              <p className="text-gray-500 max-w-sm leading-relaxed">
                Esta funcionalidade está sendo preparada para que você mude preços e adicione novos itens em segundos.
              </p>
            </div>
          )}
            ...
            <div className="max-w-4xl space-y-6">
              <Card className="p-8 shadow-xl border-orange-100">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-gray-800">Pagamento Integrado</h2>
                    <p className="text-gray-500 font-medium">Configure suas chaves do Mercado Pago.</p>
                  </div>
                </div>
                ...
                <form className="space-y-6" onSubmit={(e) => {
                  ...
                  <div className="grid md:grid-cols-1 gap-6">
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-gray-700 ml-1">Public Key</label>
                      <Input 
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-gray-700 ml-1">Access Token</label>
                      <Input 
                        type="password"
                  ...
                  <div className="flex justify-end pt-4">
                    ...
                    >
                      {saveSettingsMutation.isPending ? "Salvando..." : "Salvar Configurações de Pagamento"}
                    </Button>
                  </div>
                </form>
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-gray-800">Atendimento WhatsApp</h2>
                    <p className="text-gray-500 font-medium">Configure para onde os pedidos serão enviados.</p>
                  </div>
                </div>

                <form className="space-y-6" onSubmit={(e) => {
                  e.preventDefault();
                  updateWhatsappMutation.mutate({ key: 'whatsapp_suporte', value: whatsapp });
                }}>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 ml-1">Número do WhatsApp (Com 55 + DDD)</label>
                    <Input 
                      value={whatsapp}
                    ...
                    />
                    <p className="text-xs text-gray-400 ml-1 italic">Este é o número que receberá as mensagens de pedido do site principal.</p>
                  </div>
                  ...
                  <div className="flex justify-end pt-4">
                    ...
                    >
                      {updateWhatsappMutation.isPending ? "Salvando..." : "Salvar Numero do WhatsApp"}
                    </Button>
                  </div>
                </form>
              </Card>
