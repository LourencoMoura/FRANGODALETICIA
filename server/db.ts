...
export async function getDb() {
  if (!_db && process.env.DATABASE_URL) {
    try {
      console.log(`[Database] [Boot] Attempting connection to: ${process.env.DATABASE_URL.substring(0, 30)}...`);
      ...
      console.error("[Database] [CRITICAL ERROR] Failed during initialization:", error.message);
...
export async function savePushSubscription(customerId: number, subscription: any) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
...
export async function setAdminResetToken(email: string, token: string, expires: Date) {
  const db = await getDb();
  if (!db) return false;
